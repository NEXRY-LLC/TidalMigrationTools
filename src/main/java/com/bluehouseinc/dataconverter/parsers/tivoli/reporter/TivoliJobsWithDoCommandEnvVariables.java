...
@Log4j2
public class TivoliJobsWithDoCommandEnvVariables implements IReporter {

	String ENVVAR_PATTERN = ".*\\$\\{(.*?)\\}.*";
	...
	public void doCount(TivoliJobObject job) {
		...
		if (!StringUtils.isBlank(job.getDoCommand())) {
			...
			if (RegexHelper.matchesRegexPattern(job.getDoCommand(), ENVVAR_PATTERN)) {
