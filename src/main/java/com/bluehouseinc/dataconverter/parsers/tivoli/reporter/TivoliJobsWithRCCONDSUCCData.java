...
@Log4j2
public class TivoliJobsWithRCCONDSUCCData implements IReporter {
	...
	public void doCount(TivoliJobObject job) {
		...
		if (!StringUtils.isBlank(job.getReturnCodeSucess())) {

			jobcount.add(job.getFullPath() + " => " + job.getReturnCodeSucess());
